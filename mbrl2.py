...
class lnn(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1_R = torch.nn.Linear(4, 64)
        self.fc2_R = torch.nn.Linear(64, 64)
        self.fc3_R = torch.nn.Linear(64, 1)
    ...
    def get_acc(self, q, qdot, a):
        ...
        dV_dq = jacrev(self.get_V)(q)
        #qddot = torch.matmul(Minv,(a-c-dV_dq).unsqueeze(2)).squeeze(2)
        qddot = torch.matmul(Minv.float(), (a-c-dV_dq).unsqueeze(2).float()).squeeze(2)
        return qddot                                                                                                                                                                                                                                                                                                                 
    ...
    def forward(self, o, a):
        ...
        x = torch.cat([o, a], dim=1).float()
        y1 = F.relu(self.fc1_R(x))
        y2 = F.relu(self.fc2_R(y1))
       
        reward = self.fc3_R(y2) 
        return o_1,reward
...
def main():
    ...
    for i_ep in range(1000):
        ...
        if running_reward > -200:
            ...
            env.close()
            agent.save_param()
            with open('log/mbrl2_training_records.pkl', 'wb') as f:
